...
class ConfigUpdater(config_updater.ConfigUpdater):
    ...
    @cached_property
    def args(self):
        return alas_device_lock(read_file(filepath_args(mod_name="aaa")))

    def read_file(self, config_name, is_template=False):
        old = read_file(filepath_config(config_name, "aaa"))
        return self.config_update(old, is_template=is_template)

    @staticmethod
    def write_file(config_name, data, mod_name="aaa"):
        if deep_get(data, "Aaa.AlasLock"):
            del(data["Aaa"]["AlasLock"])
...
def alas_device_lock(args):
    if not deep_get(args, f"Aaa.AlasLock"):
        lock = {
            "type": "lock",
            "value": True
        }
        for i in range(ALASLOCK_COUNT):
            deep_set(args, f"Aaa.AlasLock.{i}", lock)
    return args
